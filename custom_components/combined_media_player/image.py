...
async def async_setup_entry(
    ...
    async_add_entities([CombinedCoverImage(entry)], update_before_add=False)
...
class CombinedCoverImage(ImageEntity):
    ...
    def __init__(self, entry: ConfigEntry) -> None:
        ...
        self._attr_suggested_object_id = f"{slugify(entry.unique_id or entry.entry_id)}_cover"
    ...
    def state(self) -> str | None:
        ...
        if active is None:
            return None
        return active.attributes.get("source") or active.attributes.get("friendly_name")

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        active = self._active_state()
        cover_url = active.attributes.get(ATTR_ENTITY_PICTURE) if active else None
        return {
            ATTR_ENTITY_PICTURE: cover_url,
